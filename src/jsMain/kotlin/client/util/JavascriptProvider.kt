...
object JavascriptProvider : PlatformProvider {
    override fun now(): ProviderDate = JavascriptDate(kotlin.js.Date(kotlin.js.Date.now()))
    ...
    override fun toDate(millis: Long): ProviderDate {
        return JavascriptDate(kotlin.js.Date(millis))
    }

    override fun toDate(input: String): ProviderDate {
        return if (input.length == "YYYY-MM-DD".length) {
            JavascriptDate(kotlin.js.Date(input + "T12:00:00"))
        } else {
            JavascriptDate(kotlin.js.Date(input))
        }
    }

    override fun toDate(year: Int, month: Int, dayOfMonth: Int): ProviderDate {
        return JavascriptDate(kotlin.js.Date(year, month, dayOfMonth))
    }
}
